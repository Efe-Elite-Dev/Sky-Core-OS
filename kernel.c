...
typedef enum {
    ...
    WIN_APP_STORE,
    WIN_INSTALL_DONE,
    WIN_AI_SYSTEM       /* AI Penceresi İçin Yeni Durum */
} ActiveWindow;
...
static void draw_terminal_window(void)
{
    ...
    draw_str(264, 260, "ai_subsystem : AKTIF (Core V1.5)", 0xFF00E5FF, 1);
...
static void draw_exe_window(void)
{
    ...
    /* UI Üzerinde AI Tetikleme Bilgisi */
    if (str_eq(aname, "Terminal")) {
        draw_str(200, 210, "AI_SUBSYSTEM: Dinlemede...", 0xFF00E5FF, 1);
    }

    draw_str(200, 230, "EXE_SUBSYSTEM: Calistiriliyor...", 0xFF00FF00, 1);
    draw_str(200, 252, "Protected Mode : AKTIF", 0xFFAAAAAA, 1);
    draw_str(200, 272, "ELF Stub       : Yuklendi", 0xFFAAAAAA, 1);
    draw_str(200, 292, "Calistirilan   :", 0xFFFFFFFF, 1);
    draw_str(340, 292, aname, 0xFF00E5FF, 1);
    draw_str(200, 320, "Status: SUCCESS", 0xFF00FF00, 1);
    draw_str(200, 345, "PID: 0x1337  MEM: 2MB  CPU: i386", 0xFF888888, 1);
...
/* AI Sistemi için Arayüz Çizici */
static void draw_ai_window(void)
{
    draw_rect(200, 200, 620, 360, 0xFF1F1A3A);
    draw_rect_outline(200, 200, 620, 360, 0xFFBD93F9);
    draw_rect(200, 200, 620, 30, 0xFF282A36);
    draw_str(215, 210, "Sky Core AI Subsystem v1.5", 0xFFBD93F9, 1);
    draw_rect(794, 204, 22, 22, 0xFFCC3333);
    draw_str(799, 207, "X", 0xFFFFFFFF, 1);

    draw_str(230, 260, ">> AI Core Status: RUNNING", 0xFF50FA7B, 1);
    draw_str(230, 290, ">> Model         : NeuralStub-32bit", 0xFFF8F8F2, 1);
    draw_str(230, 320, ">> Hız           : 0ms (Local Bare-Metal)", 0xFF8BE9FD, 1);
    draw_str(230, 360, "Sistem komutları yapay zeka alt katmanı", 0xFFFF79C6, 1);
    draw_str(230, 380, "tarafından optimize ediliyor...", 0xFFFF79C6, 1);

    draw_str(230, 520, "ESC - Alt Sistemi Kapat", 0xFF6272A4, 1);
}
...
static void render_desktop(void)
{
    ...
    switch (active_win) {
        ...
        case WIN_AI_SYSTEM:    draw_ai_window();       break; /* AI UI Tetiklenmesi */
        default: {
            int px = 240, py = 300;
...
static volatile OS_STATE g_state = STATE_DESKTOP; /* GÜNCELLEME: Doğrudan masaüstünde başla */
    ...
    if ((mb[0] & 1) && g_state == STATE_DESKTOP) {
        ...
        if (active_win != WIN_NONE) {
            ...
            if (mouse_x > 878 && mouse_x < 900 && mouse_y > 104 && mouse_y < 128)
                active_win = WIN_NONE;
            if (mouse_x > 794 && mouse_x < 816 && mouse_y > 204 && mouse_y < 226) /* AI Kapatma */
                active_win = WIN_NONE;
        }
    }
...
void kernel_main(void *mboot_ptr, uint32_t magic)
{
    ...
    g_state = STATE_DESKTOP; /* GÜNCELLEME: OOBE Atlandı, direkt masaüstü */
    refresh();
...
/* ============================================================
 * 13. STUB'LAR & ALT SİSTEMLER
 * ============================================================ */
void keyboard_init(void)       {}
void wind_subsystem_init(void) {}

/* GÜNCELLEME: AI Alt Sistemi Başlatıcı Mantığı Yazıldı */
void ai_subsystem_init(void)   
{
    if (g_state == STATE_DESKTOP) {
        active_win = WIN_AI_SYSTEM; /* Yapay zeka penceresini ekrana getir */
        refresh();
    }
}
